...
class MainWindow(QMainWindow):
    ...
    def start_analyzer(self) -> None:
        ...
        for err in errors:
            ...
            self.output_errors.setItem(row, 0, QTableWidgetItem(str(err.line)))
            self.output_errors.setItem(row, 1, QTableWidgetItem(str(err.column)))
    ...
    def _jump_to_error(self, row: int, _col: int) -> None:
        ...
        if not editor:
            return
        line_item = self.output_errors.item(row, 0)
        pos_item = self.output_errors.item(row, 1)
